...
func generateStructFromFields(opt loader.Option) error {
	...
	for _, field := range fields {
		...
		if field.Tag != nil && field.Tag.Inline {
			inlineFields, err := generateSortedStructFields(field.Type)
			if err != nil {
				return err
			}
			...
			for _, field := range inlineFields {
				fieldName, err = generateFieldName(prefix+inlinePrefix, field.CustomName)
				if err != nil {
					return err
				}
				...
				generateInlineGetter(f, pkgPath, structName, inlineStructName, fieldName, field)
			}
		} else {
...
func generateSortedStructFields(structType *loader.Type) ([]loader.StructField, error) {
	structFields, err := loader.ExtractStructFields((structType.E).(*types.Struct))
	if err != nil {
		return nil, err
	}

	var fields []loader.StructField
